...
public class MapRendererService
{
	...
	public void RenderMap(string outputPath)
	{
		...
		var projected = new Dictionary<State, List<List<(double x, double y)>>>();
			...
			foreach (var ring in state.Polygons)
			{
				...
				foreach (var p in ring)
				{
					...
					double rho = Math.Sqrt(
						ProectionAlgorithm.C -
						2 * ProectionAlgorithm.n * Math.Sin(lat)
		...
		int height)
	{
		...
		canvas.DrawText("AK", centerX - 120, centerY + 50, paint);
	}
		...
		int height)
	{
		...
		canvas.DrawText("HI", offsetX + 60, offsetY+50, paint);
	}
		...
		double maxY)
	{
		...
		foreach (var state in _states)
		{
			...
			canvas.DrawText(state.Code, (float)px, (float)py, paint);
		}
	}
}